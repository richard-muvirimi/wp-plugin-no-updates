...
class PluginVersionTest extends TestCase
{
    ...
    public function testPluginVersions(): void
    {
        ...
        self::assertTrue(version_compare(NO_UPDATES_VERSION, $rmTxtVersion, "="), "readme.txt Plugin version does not match the stable version : " . NO_UPDATES_VERSION);
        self::assertTrue(version_compare(NO_UPDATES_VERSION, $rmMdVersion, "="), "readme.md Plugin version does not match the stable tag version : " . NO_UPDATES_VERSION);
        self::assertTrue(version_compare(NO_UPDATES_VERSION, $pluginVersion, "="), "Plugin version does not match the stable tag version : " . NO_UPDATES_VERSION);
    }
	...
	private function getPluginVariable(string $name):string{
		...
		preg_match("/".$name."\s*:\s*(.*)$/mi", file_get_contents(NO_UPDATES_FILE), $matches);
	...
	private function getReadMeMdVariable(string $name):string{
		...
		preg_match("/".$name."\s?:.+\s(.*)$/miU", file_get_contents(dirname(NO_UPDATES_FILE) . DIRECTORY_SEPARATOR.$readme), $matches);
	...
	private function getReadMeTxtVariable(string $name):string{
		...
		preg_match("/".$name."\s?:\s?(.*)$/mi", file_get_contents(dirname(NO_UPDATES_FILE) . DIRECTORY_SEPARATOR.$readme), $matches);
